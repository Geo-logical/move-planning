...
class MoveDatabase:
    ...
    def get_intervals(self, item_id):
        with sqlite3.connect(self.db_path) as conn:
            cursor = conn.cursor()
            cursor.execute('''
            ...
            return [{'location': i[0], 'start_date': i[1], 'end_date': i[2]} for i in intervals]
...
default_locations = [
    ...
    "Uhaul Container", "Uncertain", "On-Island Storage"
]
#d85413
...
location_colors = {
    ...
    "Connecticut": "#66B2FF",
    "Sydney": "#99FF99",
    "Uhaul Container": "rgb(153,120,102)",
    "Sold": "#170215",
    "Trash/Donate": "#808080",
    "California": "#FFB366",
    "Baltimore": "#530953",
    "Uncertain": "rgb(250,0,0)",
    "In-Transit": "#fb09cf",
    "On-Island Storage": "rgb(26,230,49)"
}
...
app.layout = html.Div([
    dbc.Tabs([
        ...
        dbc.Tab(label="Tasks", children=[
            html.Div([
                html.H3("Tasks", style={'color': 'white'}),
                dbc.Button("Add Task", id="open-modal", color="primary", className="mb-3"),
                dash_table.DataTable(
                    id='tasks-table',
                    columns=[
                        {"name": "Name", "id": "Name"},
                        {"name": "Category", "id": "Category"},
                        {"name": "Assigned Owner", "id": "Assigned Owner"},
                        {"name": "Notes", "id": "Notes"},
                ...
                # Task Modal
                dbc.Modal([
                    dbc.ModalHeader("Add/Edit Task"),
                    dbc.ModalBody([
                        dbc.Input(id="input-name", type="text", placeholder="Task Name", className="mb-3"),
                        dbc.Input(id="input-category", type="text", placeholder="Category", className="mb-3"),
                        dbc.Input(id="input-owner", type="text", placeholder="Assigned Owner", className="mb-3"),
                        dbc.Input(id="input-notes", type="text", placeholder="Notes", className="mb-3"),
                        html.Div(id="intervals-container", children=[]),
                        dbc.Button("Add Interval", id="add-interval", color="secondary"),
                    ]),
                    dbc.ModalFooter([
        ...
        # Gantt Chart Tab
        dbc.Tab(label="Gantt", children=[
            html.Div([
                html.H3("Move Timeline", style={'color': 'white'}),
                dcc.Dropdown(
                    id='color-by-dropdown',
                    options=[
                        {'label': 'Color by Location', 'value': 'location'},
                        {'label': 'Color by Category', 'value': 'category'},
                        {'label': 'Color by Owner', 'value': 'owner'}
                    ],
                    value='location',
                    style={
                        'backgroundColor': 'white',
    ...
    html.Div(id='dummy-div', style={'display': 'none'})  # For triggering callbacks
])
...
def initialize_notes(_):
    notes = db.get_all_notes()
    return update_notes_display(None, None)  # Your existing notes display function
...
def update_table(n_clicks, name, category, owner, notes, intervals, edit_mode):
    if not n_clicks:
    ...
    return [{
        **row,
        'actions': dbc.Button(
            "✏️",
            id={"type": "edit-task-button", "index": i},
            color="warning",
            size="sm"
        )
...
@app.callback(
    [Output("note-modal", "is_open", allow_duplicate=True),
     Output("note-text-input", "value", allow_duplicate=True),
     Output("current-note-id", "value", allow_duplicate=True)],
...
def update_notes_display(save_click, delete_clicks):
    ctx = dash.callback_context
    if not ctx.triggered:
        return dash.no_update
...
@app.callback(
    [Output("modal", "is_open", allow_duplicate=True),
     ...
     Output("input-owner", "value", allow_duplicate=True),
     Output("input-notes", "value", allow_duplicate=True),
     Output("intervals-container", "children", allow_duplicate=True)],
    [Input("open-modal", "n_clicks"),
     Input("close-modal", "n_clicks"),
     Input("submit-button", "n_clicks"),
     Input({"type": "edit-task-button", "index": ALL}, "n_clicks")],
...
def toggle_modal(n1, n2, n3, edit_clicks, data, name, category, owner, notes, intervals):
    ctx = dash.callback_context
    if not ctx.triggered:
        return False, "", "", "", "", []
    ...
    if trigger_id == "open-modal":
        ...
        if name:  # Only submit if there's at least a name
            return False, "", "", "", "", []
        return True, name, category, owner, notes, intervals
    elif "edit-task-button" in trigger_id:
        try:
            button_id = json.loads(trigger_id)
            task_index = button_id['index']
            task = data[task_index]  # Get task directly by index
            if task:
                intervals_list = db.get_intervals(task['id'])
                intervals_components = []
                for interval in intervals_list:
                    intervals_components.append(create_interval_component(
                        len(intervals_components),
                        interval['location'],
                        interval['start_date'],
                        interval['end_date']
                    ))
                return True, task['Name'], task['Category'], task['Assigned Owner'], task['Notes'], intervals_components
        except Exception as e:
            print(f"Error in edit button handling: {e}")
            return False, "", "", "", "", []
    
    return False, "", "", "", "", []

def create_interval_component(index, location="", start_date=None, end_date=None):
    return html.Div([
        dbc.Label(f"Interval {index + 1}:"),
        dcc.Dropdown(
            id={"type": "interval-location", "index": index},
            options=[{"label": l, "value": l} for l in default_locations],
            value=location,
            style={
                'backgroundColor': 'white',
        ...
        dcc.DatePickerSingle(
            id={"type": "interval-start", "index": index},
            date=start_date,
            style={'marginBottom': "5px"}
        ),
        dcc.DatePickerSingle(
            id={"type": "interval-end", "index": index},
            date=end_date,
            style={'marginBottom': "5px"}
        )
    ], className="mb-3")
